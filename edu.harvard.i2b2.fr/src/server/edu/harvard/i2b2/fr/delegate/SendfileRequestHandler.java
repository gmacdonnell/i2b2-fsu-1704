		...
		for (String cid : attachment.getAllContentIDs()) {
			...
			if (dataHandler != null) {
				...
				if (!cid.equalsIgnoreCase(attachment.getSOAPPartContentID())) {
					...
					//String canonicalFilename =new File(canonicalProjectDestDir, cid+"_"+filename.getName()).getCanonicalPath(); 
					...
					//	filename.setName(canonicalFilename); //gnm
					//	bodyType.getAny().add(filename);//gnm
