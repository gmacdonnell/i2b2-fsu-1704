...
public class PMServiceDriver {
			...
			if (procStatus.getType().equalsIgnoreCase("ERROR")) {
				...
				for (ProjectType pType : projectList) {
					if (pType.getId().equalsIgnoreCase(projectId)) {
						projectType = pType;
						//gnm
						ConfigureType configType = getConfig(security);
						for (edu.harvard.i2b2.crc.datavo.pm.CellDataType cellData :configType.getCellDatas().getCellData())
						{
							if(cellData.getId().equals("FRC"))
							{
								for (ParamType param :cellData.getParam())
								{
									log.debug(param.getName());
									if (param.getName().toLowerCase().equals("destdir"))
										projectType.getParam().add( param);
									break;
								}
								break;
							}
	...
	public ConfigureType  getConfig(SecurityType security)throws I2B2Exception
	{
		...
		try {
			...
			response = new PMServiceDriver().getRoles( security);
