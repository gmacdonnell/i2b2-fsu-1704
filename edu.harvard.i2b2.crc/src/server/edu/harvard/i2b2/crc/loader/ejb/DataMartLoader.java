...
import edu.harvard.i2b2.crc.datavo.pm.ProjectsType;
import edu.harvard.i2b2.crc.datavo.pm.SearchType;
...
*/
	public void load(IUploaderDAOFactory uploaderDaoFactory, String userId, 
		...
		try {
			...
			securityType.setDomain(uploaderDaoFactory.getDataSourceLookup().getDomainId());
			securityType.setUsername(userId);
			securityType.setPassword(passWord);
			String projectID = uploaderDaoFactory.getDataSourceLookup().getNiceName();
			edu.harvard.i2b2.crc.loader.delegate.pm.PMServiceDriver PMSD = new edu.harvard.i2b2.crc.loader.delegate.pm.PMServiceDriver();
			ProjectType pmResponseUserInfo = PMSD.checkValidUser(securityType, projectID);
			String defaultPath = "";
			if(pmResponseUserInfo != null )
			{
				for (ParamType param :pmResponseUserInfo.getParam())
				{
					log.debug(param.getName());
					if (param.getName().toLowerCase().equalsIgnoreCase("destdir"))
					{
						defaultPath= param.getValue();
						break;
					}
					
				}
			}
String path = defaultPath +uploaderDaoFactory.getDataSourceLookup().getProjectPath() ;
			//gnm
			String uploadFileName = path + localUploadFile;
