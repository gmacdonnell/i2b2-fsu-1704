...
public class CallFileRepositoryUtil {
	...
	private static void uploadConceptFile(String conceptFile,  SecurityType securityType,  String projectId)  throws I2B2Exception {
		...
		//	options.setTo(new EndpointReference(frUrl));
			//GNM
			//options.setTo(new EndpointReference(ontologyUtil.getFileManagentCellUrl()+"sendfileRequest"));
			options.setTo(new EndpointReference(ontologyUtil.getFileRepositorySendFileOperationName()));
			
			
			//options.setTo(new EndpointReference("http://144.174.39.18/i2b2/services/FRService/sendfileRequest"));
			//end gnm 
